...
extension AddAssetsTVC{
    func configureWithData(data : AllAssetsData?){
        ...
        CommonUI.setUpLbl(lbl: self.euroLbl, text: "\(CommonFunction.formattedCurrency(from: Double(data?.lastPrice ?? "")))€", textColor: UIColor.grey36323C, font: UIFont.MabryProMedium(Size.Large.sizeValue()))
        CommonUI.setUpLbl(lbl: self.percentageLbl, text: "\(CommonFunction.formattedCurrency(from: Double(data?.change ?? "")))%", textColor: (Double(data?.change ?? "") ?? 0)<0 ? UIColor.RedDF5A43 : UIColor.GreenColor, font: UIFont.MabryPro(Size.Small.sizeValue()))
