...
extension AllCoinCVC{
    func configureWithData(data : PriceServiceResume?){
		...
		self.coinImgVw.sd_setImage(with: URL(string: curency.imageUrl ?? ""), completed: nil)
        ...
        CommonUI.setUpLbl(lbl: coinPercentageLbl, text: "\(data?.priceServiceResumeData.change ?? "")%", textColor: (Double(data?.priceServiceResumeData.change ?? "") ?? 0)<0 ? UIColor.Red_500 : UIColor.GreenColor, font: UIFont.MabryPro(Size.Medium.sizeValue()))
    }
}
