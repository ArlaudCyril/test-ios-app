...
extension AllCoinCVC{
    func configureWithData(data : AllAssetsData?,assetDetail : AssetDetailData?){
        ...
        CommonUI.setUpLbl(lbl: coinPercentageLbl, text: "\(CommonFunction.formattedCurrency(from: Double(data?.change ?? "")))%", textColor: (Double(data?.change ?? "") ?? 0)<0 ? UIColor.RedDF5A43 : UIColor.GreenColor, font: UIFont.MabryPro(Size.Medium.sizeValue()))
    }
}
